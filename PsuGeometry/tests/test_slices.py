...
runs = [
[1,-1,'seismic',True,False],
[3,-2,'inferno',False,False]
...
for pdbCode in pdbCodes:
            ...
            if True:
                ...
                if len(centralxyz) > 0 and len(linearxyz) and len(planarxyz) > 0:
                    ...
                    for run in runs:
                        ...
                        sfc = georep.addDensitySlice(pdbCode,Fos,Fcs,length,gaps,central,linear,planar,palette=palette,title=title,logged=logged,centre=zero)
                        ...
                        if c == 0:
                            ...
                            #FoFc = 'Fo'
                            #sfc = georep.addDensitySlice(pdbCode, 1, 0, length, gaps, central, linear, planar,palette=palette, title=title, logged=logged, centre=zero)
                            #FoFc = 'Fc'
                            #sfc = georep.addDensitySlice(pdbCode, 0, 1, length, gaps, central, linear, planar,palette=palette, title=title, logged=logged, centre=zero)
    ...
    georep.printToHtml(pdbCode.upper() + ' Peptide Bond Density/Difference Overlay', 3,  pdbCode + '_overlay')
